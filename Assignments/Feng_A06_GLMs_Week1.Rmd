...
```{r}
#5
TN_seasonally <- PeterPaul.surface93_96 %>%
  select(lakename, month, year4, tn_ug)
TN_seasonally

TN_seasonally.anova.2way <- aov(data = TN_seasonally, tn_ug ~ lakename + month)
summary(TN_seasonally.anova.2way)
...
```{r}
#7
plot_TN <- ggplot(PeterPaul.surface93_96, aes(x = year4, y = tn_ug, color = lakename)) +
  geom_boxplot() +
...
plot_TP <- ggplot(PeterPaul.surface93_96, aes(x = month, y = tp_ug, color = lakename)) + # month does not work
  geom_boxplot() +
  labs(x=expression(paste("Month"))) + 

...
```{r}
#3
ChemPhy_Wrangle <- filter(ChemPhy, daynum %in% c(182:213)) %>%
...
> 73.99% variation of a dependent variable is explained by the independent variable(s) in a regression model.
...
> Answer: There is a significant interaction between depth and lakename. 78.59% proportion of variance in temperature can be explained.
...
ggplot(ChemPhy_Wrangle, aes(x = depth, y = temperature_C, color = lakename)) +
  ...
  ylim(0,35) +
  geom_smooth(method = "lm", se= FALSE)+ # have two line to present your results, and not include random factor
  #geom_abline(intercept = -97.72, slope = 0.06) + #geom_abline need intercept and slope value
  # make it look better

...
> Add notes here: ecosystem health(nutrient), why interested temperature (climate change with the log time), if we can understand the machnism behind the issue, what organism can live, how it impact species, this studies can influence policies
...
# matrix filtering ("," rows=0, column=every single column)
...
# 除去其他的湖（用&) 剩下的是Paul or Peter lake

NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
# %in% c() is an include operator
# %in% checks whether or not the object is contained in the other object
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #"," 想当于&
...
NTL.phys.data.T90_99 <- filter(NTL.phys.data, 
                               lakename == "Tuesday Lake" & 
                               year4 %in% c(1990:1999))
dim(NTL.phys.data.T90_99)
```
Question: Why don't we filter using row numbers?

> Answer: want to filter based on attribute instead of rows(dataset in rows may change)
...
# midsummer and shollow depth; East long lake & Hummingbird Lake
...
NTL.phys.data.temps1 <- select(NTL.phys.data, -lakename) #not include lakename
```
### Mutate
...
#use NTL.phys.data dataset, then filter "Paul Lake" and "Peter Lake", then select X column to Y coloun, then add a column use "mutate"
```

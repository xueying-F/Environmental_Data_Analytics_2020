...
library(tidyverse)
options(scipen = 4) # this value determines how likely it is that scientific notation will be triggered. So to prevent scientific notation, simply use a large positive value like 999
...
> As greater depths, irradiance decrease
> Depth accounts for 31% of variance in lake irradiance
> Irradiance deceases significantly with decreasing depth (linear regression, R2=0.31, df= 15,449 , p<0.0001)
> For each 1m increase in depth, irradiance decrease by 95 unit
...
par(mfrow = c(1,1)) # set plot window 1 by 1
```
...
irradiancebydepth <- 
  ...
  #ylim(0, 2000) + # limite number on y axis
  #xlim(0,13)+
  geom_point() 
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) # remove the irradiantwater column that value =0, and filter irradianceWater < 5000)
...
# Add a line and standard error for the linear regression (make a new plot)
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + # two variables could use geom_smooth
  scale_y_log10() + # it will give the same X axis label name  !!!!!!!! pay attentionon interprete log
  geom_point() 
print(irradiancebydepth2) ssss

irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = log(irradianceWater))) + # log on y can see the x axis label change
  geom_smooth(method = "lm") + # two variables could use geom_smooth
  #scale_y_log10() +
  geom_point() 
print(irradiancebydepth2) 

# SE can also be removed (make a new plot
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(pch=1, alpha =o.5, color = "lightblue") +   # pch= plotting ‘character’, i.e., symbol to use.
    scale_y_log10() +  ## scale by log, so the graph is correct
    labs(x="Depth (m)") +
    labs(y=expression(Irradiance (W*m^-2)))+ 
    geom_smooth(method = 'lm', se = FALSE, color = "black") # color the line #se= Display confidence interval around smooth? 
print(irradiancebydepth2)

# Make the graph attractive (revised on above one)
...
``` {r}
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth, 
         method = "spearman", exact = FALSE) # exact= a logical indicating whether an exact p-value should be computed
```
...
TPregression <- lm(data = subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
                   tp_ug ~ depth + dissolvedOxygen) # depend on depth
...
TPplot <- ggplot(subset(PeterPaul.chem.nutrients, lakename == "Paul Lake"), 
  ...
  geom_point() +
  xlim(0, 20) +
  scale_color_viridis(option = "magma", direction = -1) #deriction of color
print(TPplot)  # difficult model , hard to visualize
...
corrplot(PeterPaulCorr, method = "ellipse") #the visualization method of correlation matrix to be used
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #颜色越浅 correlation越少
```
...
```{r} 
Paul.naomit <- PeterPaul.chem.nutrients %>%  # create a new dataset: only paul lake, remove na
  filter(lakename == "Paul Lake") %>%
  na.omit() 

TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen +  # run a linear model
              temperature_C + tn_ug + po4) 
step(TPAIC)  #step: Select a formula-based model by AIC.

TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug) # Creat a new linear model (final model selection)
summary(TPmodel)

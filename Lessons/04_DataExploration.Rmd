...
#./go one file in use
#../ go out one folder,then click tab
# knit directary
#~/ click tab
USGS.flow.data <- read.csv("../Data/Raw/NEON_NIWO_Litter_trapdata_raw.csv")
...
# Rename columns(have to list all the columns)
# only change in data frame in R, not in excel
...
colnames(USGS.flow.data)[1] <- c("agency") #change the first column name
                              
str(USGS.flow.data)
dim(USGS.flow.data) #dimension
length(USGS.flow.data) #how many columns
nrow(USGS.flow.data) #how mant rows

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #matrix: line 30000to30005, and column 3&8&14
...
#na.omit  only conatin the completed data set
#remove N/A the blank goona gone
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data.complete$discharge.mean)
sd(USGS.flow.data.complete$discharge.mean)
summary(USGS.flow.data.complete$discharge.mean) # no N/A in results
...
today <- Sys.Date() #system date year-month-date
...
#191231 means 2019/12/31; if num >191231, put 19 in front of
...
```{r}
write.csv(USGS.flow.data, file = "./Data/Raw/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE)

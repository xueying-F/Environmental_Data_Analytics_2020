...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean)# only frop na in discharge.mean 
                          # na.exclude, remove na from entire dataframe
...
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365) #ts: time series
                                                                    #[[8]]: column 8 of data set
#time series do not use $, and we nee dto select exact column by []this way.
# this will show in values rather than Data.
...
### stl:  Decompose a time series into seasonal, trend and irregular components using loess (locally estimated scatterplot smoothing)

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed) # from year 0 to year 35 
                        # add seasonal, trend, remainder = data set (trend and seasonal scale is too small, so remainder and data are similar)
                        # seasonal means the cycle(could be daliy, season, day and night) repeat 35 times
...
EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
                      Observed = EnoDischarge.late$discharge.mean,     ## adds new variables
...
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000) #sample_n for shapiro.test
EnoDischarge.late.subsample <- sample_n(EnoDischarge.late, 5000)

shapiro.test(EnoDischarge.early.subsample$discharge.mean) #是否normality
shapiro.test(EnoDischarge.late.subsample$discharge.mean)

var.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean) #是否equal variances
...
<<<<<<< HEAD
> median daliy charge in Eno Rive from 1827-1971 was significantly higher than mean daily discharger from 1985-2019 (wilcox.test, W = 1.17e+08, p<0.0001)
>we can gapy boxplot and got them seperately.
...
> aggregation:The mathematical computation applied such as summing all the values, computing the average or picking the highest.

> subsampling: The function subsample can be used to build an appropriate weight matrix to be used with stabsel 

> interpolation: Generic function for replacing each NA with interpolated values.
...
EnoDischarge.late.monthly <- EnoDischarge.late %>%
  ...
  group_by(Year, Month) %>%  # the only column gonna be group by and summarise column
  summarise(Discharge = mean(discharge.mean))  #calculate the mean monthly discharge
...
EnoDischarge.late.monthly.ts <- ts(EnoDischarge.late.monthly$Discharge, frequency = 12, 
                        start = c(1985, 10, 1), end = c(2019, 12, 1)) #frquency: 	the number of observations per unit of time.
...
> Discharge has changed over the course of measurement while incorporating the seasonal component. no significant in seasonal (could be month, day, or season) monotonic trend.(report z and p value)
